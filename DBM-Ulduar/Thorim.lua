...
local warnNova          		= mod:NewSpellAnnounce(312904, 1)    -- Нова
local warnVolley          		= mod:NewSpellAnnounce(312902, 1)    -- Залп ледяных стрел
local warnUnbalancingStrike		= mod:NewTargetAnnounce(312898, 1)   -- Деформирующий удар
...
local specWarnUnbalancingStrike	        = mod:NewSpecialWarningTaunt(312898, "Tank", nil, nil, 1, 2)
local specWarnUnbalancingStrikeSelf	= mod:NewSpecialWarningDefensive(312898, mod:IsTank())
...
local timerLightningCharge2	 	= mod:NewCDTimer(12, 312895, nil, nil, nil, 2, nil)   -- Цепная молния
...
local timerUnbalancingStrike	        = mod:NewTargetTimer(15, 312898, nil, "Tank", nil, 3, nil, DBM_CORE_TANK_ICON)     --дисбаланс
local timerHardmode			= mod:NewTimer(175, "TimerHardmode")
local timerUnbalancingStrikeCD	 	= mod:NewCDTimer(20, 312898, nil, "Tank|Healer", nil, 3, nil, DBM_CORE_TANK_ICON)  --дисбаланс
