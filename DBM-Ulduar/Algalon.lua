...
local announcePhasePunch		= mod:NewAnnounce("WarningPhasePunch", 4, 313039, mod:IsHealer() or mod:IsTank())

local specwarnStarLow			= mod:NewSpecialWarning("warnStarLow", mod:IsHealer() or mod:IsTank())
local specWarnPhasePunch		= mod:NewSpecialWarningStack(313033, nil, 4)
local specWarnBigBang			= mod:NewSpecialWarningSpell(313034)
...
local timerNextBigBang			= mod:NewNextTimer(90.5, 313034)
...
local timerCastCosmicSmash		= mod:NewCastTimer(4.5, 313037)
local timerPhasePunch			= mod:NewBuffActiveTimer(45, 313033)
local timerNextPhasePunch		= mod:NewNextTimer(16, 313033)
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(64584, 64443, 312681, 313034) then 	-- Big Bang
...
function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpellID(65108, 64122, 312686, 313039) then 	-- Black Hole Explosion
		announceBlackHole:Show()
		warned_star = false
	elseif args:IsSpellID(64598, 62301, 313036, 312683) then	-- Cosmic Smash
...
function mod:SPELL_AURA_APPLIED(args)
	if args:IsSpellID(64412, 312680, 313033) then
